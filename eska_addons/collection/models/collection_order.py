...
class CollectionOrder(models.Model):
    ...
    def _compute_deposited(self):
        for collection in self:
            collection.total_deposited = sum(
                collection.payment_ids.filtered(
                    lambda x: x.state in ['draft', 'posted'] and x.amount_company_currency_signed > 0.0).mapped('amount'))

    @api.depends('transfer_payment_ids.amount', 'currency_id')
    def _compute_transferred(self):
        for collection in self:
            collection.total_transferred = sum(
                collection.transfer_payment_ids.filtered(
                    lambda x: x.state in ['draft', 'posted'] and x.amount_company_currency_signed > 0.0).mapped('amount'))

    @api.depends('total_collected', 'total_deposited', 'collection_line_ids.payment_ids.line_ids.reconciled')
    def _compute_states(self):
        for rec in self:
            if rec.total_deposited > 0 or rec.total_transferred:
                reconcile = rec.mapped('collection_line_ids.payment_ids.line_ids').filtered(lambda r: not r.reconciled)
                if reconcile:
