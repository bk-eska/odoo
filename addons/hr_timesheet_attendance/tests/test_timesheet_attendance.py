...
class TestTimesheetAttendance(TestCommonTimesheet):
    ...
    def test_timesheet_attendance_report(self):
        ...
        report_data = self.env['hr.timesheet.attendance.reports']._read_group(
            [('user_id', '=', self.user_employee.id),
            ('date', '>=', datetime(2022, 2, 9, 8, 0)), ('date', '<=', datetime(2022, 2, 9, 16, 0))],
            ['total_timesheet', 'total_attendance', 'total_difference'],
            ['user_id'],
        )[0]
        self.assertEqual(report_data['total_timesheet'], 6.0, "Total timesheet in reports should be 4.0")
        self.assertEqual(report_data['total_attendance'], 8.0, "Total attendance in reports should be 8.0")
