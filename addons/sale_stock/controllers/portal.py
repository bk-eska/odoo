...
class SaleStockPortal(CustomerPortal):
    ...
    def portal_my_picking_report(self, picking_id, access_token=None, **kw):
        ...
        # print reports with sudo, since it require access to product, taxes, payment term etc.. and portal does not have those access rights.
        pdf = request.env['ir.actions.reports'].sudo()._render_qweb_pdf('stock.action_report_delivery', [picking_sudo.id])[0]
        pdfhttpheaders = [
