...
class AccountReport(models.Model):
    ...
    def _compute_report_option_filter(self, field_name, default_value=False):
        # We don't depend on the different filter fields on the root report, as we don't want a manual change on it to be reflected on all the report
        ...
        if 'country_id' in vals:
            ...
            for expression in tax_tags_expressions:
                ...
                if all(report in self for report in tag_reports):
                    # Only report in self are using these tags; let's change their country
