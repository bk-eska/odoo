...
class IrActionsReport(models.Model):
    ...
    printing_action_ids = fields.One2many(
        comodel_name="printing.reports.xml.action",
    ...
    def behaviour(self):
        ...
        if print_action:
            # For some reason design takes reports defaults over
    ...
    def print_document(self, record_ids, data=None):
        ...
        if not report_type:
            raise exceptions.UserError(
                _("This reports type (%s) is not supported by direct printing!")
        ...
        if not printer:
            raise exceptions.UserError(_("No printer configured to print this reports."))
    ...
    def _can_print_report(self, behaviour, printer, document):
        """Predicate that decide if reports can be sent to printer

        If you want to prevent `render_qweb_pdf` to send reports you can set
    ...
    def _render_qweb_pdf(self, report_ref, res_ids=None, data=None):
        ...
        If the action configured on the reports is server, it prints the
