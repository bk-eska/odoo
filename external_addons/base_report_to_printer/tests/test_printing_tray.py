...
class TestPrintingTray(TransactionCase):
    ...
    def test_report_behaviour(self):
        """It should add the selected tray in the reports data"""
        ir_report = self.env["ir.actions.reports"].search([], limit=1)
        report = self.env["printing.reports.xml.action"].create(
