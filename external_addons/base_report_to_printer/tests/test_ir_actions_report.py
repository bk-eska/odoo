...
class TestIrActionsReportXml(TransactionCase):
    def setUp(self):
        ...
        self.Model = self.env["ir.actions.reports"]
        self.vals = {}

        self.report = self.env["ir.actions.reports"].search([], limit=1)
    ...
    def new_printing_action(self):
        return self.env["printing.reports.xml.action"].create(
            {
    ...
    def test_print_action_for_report_name_returns_if_no_report(self):
        """It should return empty dict when no matching reports"""
        with mock.patch("%s._get_report_from_name" % model) as mk:
            expect = "test"
    ...
    def test_behaviour_report_values(self):
        """It should return the action and printer from reports"""
        report = self.Model.search([], limit=1)
        self.env.user.printing_action = "client"
    ...
    def test_behaviour_printing_action_on_wrong_report(self):
        ...
        printing_action.report_id = self.env["ir.actions.reports"].search(
    ...
    def test_print_tray_behaviour(self):
        ...
        """
        report = self.env["ir.actions.reports"].search([], limit=1)
        action = self.env["printing.reports.xml.action"].create(
    ...
    def test_onchange_printer_tray_id_empty(self):
        action = self.env["ir.actions.reports"].new({"printer_tray_id": False})
    ...
    def test_onchange_printer_tray_id_not_empty(self):
        ...
        action = self.env["ir.actions.reports"].new({"printer_tray_id": tray.id})
