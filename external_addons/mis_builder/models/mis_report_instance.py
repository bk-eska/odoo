...
class MisReportInstancePeriodSum(models.Model):
    ...
    _name = "mis.reports.instance.period.sum"
    _description = "MIS Report Instance Period Sum"

    period_id = fields.Many2one(
        comodel_name="mis.reports.instance.period",
...
class MisReportInstancePeriod(models.Model):
    """A MIS reports instance has the logic to compute
    a reports template for a given date period.
    ...
    _name = "mis.reports.instance.period"
    ...
    mode = fields.Selection(
        [
            ...
            (MODE_REL, "Relative to reports base date"),
    ...
    report_instance_id = fields.Many2one(
        comodel_name="mis.reports.instance",
    ...
    subkpi_ids = fields.Many2many("mis.reports.subkpi", string="Sub KPI Filter")
    ...
    source_sumcol_ids = fields.One2many(
        comodel_name="mis.reports.instance.period.sum",
    ...
    source_cmpcol_from_id = fields.Many2one(
        comodel_name="mis.reports.instance.period", string="versus"
    )
    source_cmpcol_to_id = fields.Many2one(
        comodel_name="mis.reports.instance.period", string="Compare"
    )
    allowed_cmpcol_ids = fields.Many2many(
        comodel_name="mis.reports.instance.period", compute="_compute_allowed_cmpcol_ids"
    )
    ...
    _sql_constraints = [
        ...
        (
            ...
            "Period name should be unique by reports",
        ),
    ]
        ...
        for record in self:
            if record.source == SRC_ACTUALS:
                if not record.report_instance_id.report_id:
                    raise UserError(
                        _(
                            "Please select a reports template and/or "
                            "save the reports before adding columns."
                        )
                    )
                # use the default model defined on the reports template
    ...
    def _get_additional_move_line_filter(self):
        ...
        The default filter is obtained from the reports-instance-level
        ...
        if not self.source_aml_model_name:
            ...
            return []
        # First get the reports-level filter domain.
        ...
        for rec in self:
            if rec.source == SRC_CMPCOL:
                ...
                if (
                    ...
                    raise ValidationError(
                        _("Columns to compare must belong to the same reports " "in %s")
                        % rec.name
                    )

    def copy_data(self, default=None):
        if self.source == SRC_CMPCOL:
            # While duplicating a MIS reports instance, comparison columns are
...
class MisReportInstance(models.Model):
    """The MIS reports instance combines everything to compute
    a MIS reports template for a set of periods."""
    ...
    report_id = fields.Many2one("mis.reports", required=True, string="Report")
    ...
    currency_id = fields.Many2one(
        ...
        string="Currency",
        help="Select target currency for the reports. "
    ...
    analytic_domain = fields.Text(
        default="[]",
        help=(
            "A domain to additionally filter move lines considered in this reports. "
    ...
    widget_show_filters = fields.Boolean(
        ...
        help="Show the filter bar in the reports widget.",
    )
    widget_show_settings_button = fields.Boolean(
        default=False,
        string="Show settings button",
        help="Show the settings button in the reports widget.",
    )
    widget_show_pivot_date = fields.Boolean(
        default=False,
        string="Show Pivot Date",
        help="Show the Pivot Date in the reports widget filter bar.",
    )
    widget_search_view_id = fields.Many2one(
        ...
        help="Search view to customize the filter box in the reports widget.",
    )
    ...
    def _get_filter_domain(self, source_aml_model_name):
        ...
        It combines
          - the draft/posted filter (if the move line source has a parent_state
            field).
          - the analytic domain field configured on this reports instance
        ...
        # reports-level analytic domain filter
        domain.extend(ast.literal_eval(self.analytic_domain))
    ...
    def get_views(self, views, options=None):
        ...
        if (
            ...
            and context.get("active_model") == "mis.reports.instance"
        ):
    ...
    def preview(self):
        ...
        return {
            ...
            "res_model": "mis.reports.instance",
            "res_id": self.id,
            "view_mode": "form",
            "view_id": view_id.id,
            "target": "current",
    ...
    def _compute_matrix(self):
        """Compute a reports and return a KpiMatrix.

        The key attribute of the matrix columns (KpiMatrixCol)
        is guaranteed to be the id of the mis.reports.instance.period.
        """
        self.ensure_one()
    ...
    def drilldown(self, arg):
        ...
        if period_id and expr and AEP.has_account_var(expr):
            period = self.env["mis.reports.instance.period"].browse(period_id)
            aep = AEP(
    ...
    def _get_drilldown_action_name(self, arg):
        ...
        kpi = self.env["mis.reports.kpi"].browse(kpi_id)
        period_id = arg.get("period_id")
        period = self.env["mis.reports.instance.period"].browse(period_id)
        account_id = arg.get("account_id")
