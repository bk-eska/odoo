...
class MisReportKpi(models.Model):
    """A KPI is an element (ie a line) of a MIS reports.

    In addition to a name and description, it has an expression
    to compute it based on queries defined in the MIS reports.
    ...
    KPI's have a sequence and are ordered inside the MIS reports.
    """

    _name = "mis.reports.kpi"
    ...
    expression_ids = fields.One2many(
        comodel_name="mis.reports.kpi.expression",
        inverse_name="kpi_id",
        copy=True,
        string="Expressions",
    )
    ...
    auto_expand_accounts_style_id = fields.Many2one(
        ...
        comodel_name="mis.reports.style",
        required=False,
    )
    style_id = fields.Many2one(
        string="Style", comodel_name="mis.reports.style", required=False
    )
    ...
    report_id = fields.Many2one("mis.reports", required=True, ondelete="cascade")
    ...
    def _inverse_expression(self):
        for kpi in self:
            ...
            if kpi.expression_ids:
                ...
                expression = self.env["mis.reports.kpi.expression"].new(
                    {"name": kpi.expression}
                )
...
class MisReportSubkpi(models.Model):
    _name = "mis.reports.subkpi"
    _description = "MIS Report Sub-KPI"
    _order = "sequence, id"

    sequence = fields.Integer(default=1)
    report_id = fields.Many2one(
        comodel_name="mis.reports", required=True, ondelete="cascade"
    )
    name = fields.Char(required=True)
    description = fields.Char(required=True, translate=True)
    expression_ids = fields.One2many("mis.reports.kpi.expression", "subkpi_id")
...
class MisReportKpiExpression(models.Model):
    """A KPI Expression is an expression of a line of a MIS reports Kpi.
    It's used to compute the kpi value.
    """

    _name = "mis.reports.kpi.expression"
    ...
    kpi_id = fields.Many2one("mis.reports.kpi", required=True, ondelete="cascade")
    # TODO FIXME set readonly=True when onchange('subkpi_ids') below works
    subkpi_id = fields.Many2one("mis.reports.subkpi", readonly=False, ondelete="cascade")
...
class MisReportQuery(models.Model):
    """A query to fetch arbitrary data for a MIS reports.
...
class MisReport(models.Model):
    """A MIS reports template (without period information)

    The MIS reports holds:
    ...
    * a list of KPI to be evaluated based on the variables resulting
    ...
    style_id = fields.Many2one(string="Style", comodel_name="mis.reports.style")
    query_ids = fields.One2many(
        "mis.reports.query", "report_id", string="Queries", copy=True
    )
    kpi_ids = fields.One2many("mis.reports.kpi", "report_id", string="KPI's", copy=True)
    subkpi_ids = fields.One2many(
        "mis.reports.subkpi", "report_id", string="Sub KPI", copy=True
    )
    subreport_ids = fields.One2many(
        "mis.reports.subreport", "report_id", string="Sub reports", copy=True
    )
    all_kpi_ids = fields.One2many(
        comodel_name="mis.reports.kpi",
        compute="_compute_all_kpi_ids",
        help="KPIs of this reports and subreports.",
    )
    ...
    def _on_change_subkpi_ids(self):
        """Update kpi expressions when subkpis change on the reports,
    ...
    def copy(self, default=None):
        ...
        for subkpi in self.subkpi_ids:
            # search expressions linked to subkpis of the original reports
            exprs = self.env["mis.reports.kpi.expression"].search(
                [("kpi_id.report_id", "=", new.id), ("subkpi_id", "=", subkpi.id)]
            )
            # and replace them with references to subkpis of the new reports
    ...
    def _declare_and_compute_period(
        ...
        """Evaluate a reports for a given period, populating a KpiMatrix.
    ...
    def evaluate(
        ...
        """Simplified method to evaluate a reports over a time period.
