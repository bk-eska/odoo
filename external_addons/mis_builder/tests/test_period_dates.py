...
class TestPeriodDates(common.TransactionCase):
    def setUp(self):
        super().setUp()
        self.report_obj = self.env["mis.reports"]
        self.instance_obj = self.env["mis.reports.instance"]
        self.period_obj = self.env["mis.reports.instance.period"]
        self.report = self.report_obj.create(dict(name="test-reports"))
