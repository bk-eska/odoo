...
class TestMisReportInstance(common.HttpCase):
    """Basic integration test to exercise mis.reports.instance.
    ...
    def setUp(self):
        ...
        # create a reports with 2 subkpis and one query
        self.report = self.env["mis.reports"].create(
            dict(
                name="test reports",
                subkpi_ids=[
        ...
        # create another reports with 2 subkpis, no query
        self.report_2 = self.env["mis.reports"].create(
            dict(
                name="another test reports",
                subkpi_ids=[
                    (
                        0,
                        0,
                        dict(
                            ...
                            description="subkpi 1, reports 2",
                            sequence=1,
                        ),
                    ),
                    (
                        ...
                        dict(
                            ...
                            description="subkpi 2, reports 2",
                            sequence=2,
                        ),
                    ),
                ],
        ...
        # Third reports, 2 subkpis, no query
        self.report_3 = self.env["mis.reports"].create(
            dict(
                name="test reports 3",
                subkpi_ids=[
                    (
                        ...
                        dict(
                            ...
                            description="subkpi 1, reports 3",
                            sequence=1,
                        ),
                    ),
                    (
                        ...
                        dict(
                            ...
                            description="subkpi 2, reports 3",
                            sequence=2,
                        ),
                    ),
                ],
        ...
        self.kpi1 = self.env["mis.reports.kpi"].create(
            dict(
        ...
        self.kpi2 = self.env["mis.reports.kpi"].create(
            dict(
                report_id=self.report.id,
                description="kpi 2",
                name="k2",
        ...
        self.env["mis.reports.kpi"].create(
            dict(
                report_id=self.report.id,
                description="kpi 4",
                name="k4",
        ...
        self.report_instance = self.env["mis.reports.instance"].create(
            dict(
                name="test instance",
                report_id=self.report.id,
        ...
        # same for reports 2
        self.report_instance_2 = self.env["mis.reports.instance"].create(
            dict(
        ...
        # and for reports 3
        self.report_instance_3 = self.env["mis.reports.instance"].create(
            dict(
    ...
    def test_kpi_name_get_name_search(self):
        r = self.env["mis.reports.kpi"].name_search("k1")
        self.assertEqual(len(r), 1)
        self.assertEqual(r[0][0], self.kpi1.id)
        self.assertEqual(r[0][1], "kpi 1 (k1)")
        r = self.env["mis.reports.kpi"].name_search("kpi 1")
        self.assertEqual(len(r), 1)
        self.assertEqual(r[0][0], self.kpi1.id)
        self.assertEqual(r[0][1], "kpi 1 (k1)")

    def test_kpi_expr_name_get_name_search(self):
        r = self.env["mis.reports.kpi.expression"].name_search("k1")
        self.assertEqual(
            [i[1] for i in r],
            ["kpi 1 / subkpi 1 (k1.sk1)", "kpi 1 / subkpi 2 (k1.sk2)"],
        )
        r = self.env["mis.reports.kpi.expression"].name_search("k1.sk1")
        self.assertEqual([i[1] for i in r], ["kpi 1 / subkpi 1 (k1.sk1)"])
        r = self.env["mis.reports.kpi.expression"].name_search("k4")
