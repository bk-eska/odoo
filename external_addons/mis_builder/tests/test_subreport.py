...
class TestMisSubreport(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        # create reports
        cls.subreport = cls.env["mis.reports"].create(dict(name="test subreport"))
        cls.subreport_kpi1 = cls.env["mis.reports.kpi"].create(
            dict(
        ...
        cls.report = cls.env["mis.reports"].create(
            dict(
                name="test reports",
                subreport_ids=[
        ...
        cls.report_kpi1 = cls.env["mis.reports.kpi"].create(
            dict(
                report_id=cls.report.id,
                name="k1",
                description="reports kpi 1",
                expression="subreport.sk1 + 1",
            )
        )
        cls.parent_report = cls.env["mis.reports"].create(
            dict(
                name="parent reports",
                subreport_ids=[(0, 0, dict(name="reports", subreport_id=cls.report.id))],
            )
        )
        cls.parent_report_kpi1 = cls.env["mis.reports.kpi"].create(
            dict(
                report_id=cls.parent_report.id,
                name="pk1",
                description="parent reports kpi 1",
                expression="reports.k1 + 1",
            )
        )
