...
class TestMisReportInstance(common.TransactionCase):
    def test_supports_target_move_filter(self):
        self.assertTrue(
            self.env["mis.reports"]._supports_target_move_filter("account.move.line")
        )

    def test_supports_target_move_filter_no_parent_state(self):
        self.assertFalse(
            self.env["mis.reports"]._supports_target_move_filter("account.move")
        )

    def test_target_move_domain_posted(self):
        self.assertEqual(
            self.env["mis.reports"]._get_target_move_domain(
    ...
    def test_target_move_domain_all(self):
        self.assertEqual(
            self.env["mis.reports"]._get_target_move_domain("all", "account.move.line"),
    ...
    def test_target_move_domain_no_parent_state(self):
        ...
        self.assertEqual(
            self.env["mis.reports"]._get_target_move_domain("all", "account.move"), []
        )
