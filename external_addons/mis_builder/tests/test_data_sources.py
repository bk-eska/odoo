...
class TestMisReportInstanceDataSources(common.TransactionCase):
    ...
    def setUp(self):
        ...
        # create reports
        self.report = self.env["mis.reports"].create(dict(name="test reports"))
        ...
        self.instance = self.env["mis.reports.instance"].create(
            dict(name="test instance", report_id=self.report.id, comparison_mode=True)
        )
        self.p1 = self.env["mis.reports.instance.period"].create(
            dict(
                name="p1",
        ...
        self.p2 = self.env["mis.reports.instance.period"].create(
            dict(
                name="p2",
                report_instance_id=self.instance.id,
    ...
    def test_sum(self):
        self.psum = self.env["mis.reports.instance.period"].create(
            dict(
                name="psum",
                report_instance_id=self.instance.id,
                mode=MODE_NONE,
    ...
    def test_cmp(self):
        self.pcmp = self.env["mis.reports.instance.period"].create(
            dict(
                name="pcmp",
                report_instance_id=self.instance.id,
                mode=MODE_NONE,
