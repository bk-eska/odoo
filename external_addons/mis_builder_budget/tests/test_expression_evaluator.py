...
class TestBudgetAwareExpressionEvaluator(TransactionCase):
    ...
    def test_one_expression(self):
        evaluator = self._make_evaluator()
        kpi = self.env["mis.reports.kpi"].new({"name": "thekpi"})
        expr = self.env["mis.reports.kpi.expression"].new(
    ...
    def test_two_expressions(self):
        evaluator = self._make_evaluator()
        kpi = self.env["mis.reports.kpi"].new({"name": "thekpi"})
        expr1 = self.env["mis.reports.kpi.expression"].new(
            {"kpi_id": kpi.id, "name": "a"}
        )
        expr2 = self.env["mis.reports.kpi.expression"].new(
    ...
    def test_two_expressions_with_budget(self):
        kpi = self.env["mis.reports.kpi"].new({"name": "thekpi", "budgetable": True})
        expr1 = self.env["mis.reports.kpi.expression"].new(
            {"kpi_id": kpi.id, "name": "a"}
        )
        expr2 = self.env["mis.reports.kpi.expression"].new(
