...
class StockReportByLocationPrepare(models.TransientModel):
    _name = "stock.reports.quantity.by.location.prepare"
    ...
    def open(self):
        ...
        action = {
            ...
            "res_model": "stock.reports.quantity.by.location",
    ...
    def _compute_stock_report_by_location(self):
        ...
        for loc in self.location_ids:
            ...
            recs = self.env["stock.reports.quantity.by.location"].create(vals_list)
        return recs.ids


class StockReportQuantityByLocation(models.TransientModel):
    _name = "stock.reports.quantity.by.location"
    _description = "Stock Report By Location"

    wiz_id = fields.Many2one(comodel_name="stock.reports.quantity.by.location.prepare")
