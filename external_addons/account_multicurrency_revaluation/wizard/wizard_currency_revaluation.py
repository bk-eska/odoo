...
class WizardCurrencyRevaluation(models.TransientModel):
    ...
    def _get_default_label(self):
        return "%(currency)s %(account)s %(rate)s %(inverse)s currency revaluation"
            ...
            for partner_id, by_partner in by_account.items():
                for currency_id, lines in by_partner.items():
                    ...
                    diff_balances = self.with_context(
                        currency_rate_type_from=account.currency_revaluation_rate_type_id.id,
                        currency_rate_type_to=account.currency_revaluation_rate_type_id.id
