...
def post_init_hook(cr, registry):
    ...
    Add XSD Validation Schema for a demo reports if it's in the system.
    Demo data records are always created with `noupdate == True` and render of
    tag `reports` doesn't support new `ir.actions.reports` field `xsd_schema`.
    ...
    report_domain = [
        ("report_name", "=", "report_xml.demo_report_xml_view")  # reports tech name
    ]
    demo_report = env["ir.actions.reports"].search(report_domain, limit=1)
    if demo_report:
