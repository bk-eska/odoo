...
class ReportXmlAbstract(models.AbstractModel):
    """
    Model `reports.report_xml.abstract`.

    This class provide basic methods for rendering XML reports and it's
    validation by XSD schema.
    """

    _name = "reports.report_xml.abstract"
    _description = "Abstract XML Report"

    @api.model
    def generate_report(self, ir_report, docids, data=None):
        """
        Generate and validate XML reports. Use incoming `ir_report` settings
        to setup encoding and XMl declaration for result `xml`.

        Methods:
         * `_get_rendering_context` `ir.actions.reports` - get reports variables.
         It will call `_get_report_values` of reports's class if it's exist.
         * `render_template` of `ir.actions.reports` - get reports content
         * `validate_report` - check result content

        Args:
         * ir_report(`ir.actions.reports`) - reports definition instance in Odoo
         * docids(list) - IDs of instances for those reports will be generated
         * data(dict, None) - variables for reports rendering

        Returns:
         * str - result content of reports
         * str - `"xml"`
    ...
    def validate_report(self, xsd_schema_doc, content):
        """
        Validate final reports content against value of `xsd_schema` field
        ("XSD Validation Schema") of `ir.actions.reports` via `etree` lib.

        Args:
         * xsd_schema_doc(byte-string) - reports validation schema
         * content(str) - reports content for validation

        Raises:
         * odoo.exceptions.ValidationError - Syntax of final reports is wrong
    ...
    def _get_report_values(self, docids, data=None):
        """
        Allow to generate extra variables for reports environment.
        ...
        Returns:
         * dict - extra variables for reports render
        """
        return data or {}
